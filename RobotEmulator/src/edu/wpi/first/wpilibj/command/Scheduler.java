...
/**
 * The {@link edu.wpi.first.wpilibj.command.Scheduler} is a singleton which holds the top-level running
 * commands. It is in charge of both calling the command's
 * {@link edu.wpi.first.wpilibj.command.Command#run() run()} method and to make sure that there are no two
 * commands with conflicting requirements running.
 * <p/>
 * <p>It is fine if teams wish to take control of the {@link edu.wpi.first.wpilibj.command.Scheduler}
 * themselves, all that needs to be done is to call
 * {@link edu.wpi.first.wpilibj.command.Scheduler#getInstance() Scheduler.getInstance()}.{@link edu.wpi.first.wpilibj.command.Scheduler#run() run()}
 * often to have {@link edu.wpi.first.wpilibj.command.Command Commands} function correctly. However, this is
...
public class Scheduler implements NamedSendable {
    ...
    /**
     * Returns the {@link edu.wpi.first.wpilibj.command.Scheduler}, creating it if one does not exist.
     *
     * @return the {@link edu.wpi.first.wpilibj.command.Scheduler}
     */
    ...
    /**
     * A hashtable of active {@link edu.wpi.first.wpilibj.command.Command Commands} to their
     * {@link LinkedListElement}
     */
    private Hashtable commandTable = new Hashtable();
    /**
     * The {@link edu.wpi.first.wpilibj.command.Set} of all {@link Subsystem Subsystems}
     */
    private Set subsystems = new Set();
    /**
     * The first {@link edu.wpi.first.wpilibj.command.Command} in the list
     */
    private LinkedListElement firstCommand;
    /**
     * The last {@link edu.wpi.first.wpilibj.command.Command} in the list
     */
    ...
    /**
     * A list of all {@link edu.wpi.first.wpilibj.command.Command Commands} which need to be added
     */
    ...
    /**
     * Instantiates a {@link edu.wpi.first.wpilibj.command.Scheduler}.
     */
    private Scheduler() {
    ...
    /**
     * Adds the command to the {@link edu.wpi.first.wpilibj.command.Scheduler}. This will not add the
     * {@link edu.wpi.first.wpilibj.command.Command} immediately, but will instead wait for the proper time in
     * the {@link edu.wpi.first.wpilibj.command.Scheduler#run()} loop before doing so. The command returns
     * immediately and does nothing if given null.
     * <p/>
     * <p>Adding a {@link edu.wpi.first.wpilibj.command.Command} to the {@link edu.wpi.first.wpilibj.command.Scheduler} involves the
     * {@link edu.wpi.first.wpilibj.command.Scheduler} removing any {@link edu.wpi.first.wpilibj.command.Command} which has shared
     * requirements.</p>
     *
    ...
    /**
     * Adds a button to the {@link edu.wpi.first.wpilibj.command.Scheduler}. The {@link edu.wpi.first.wpilibj.command.Scheduler} will poll
     * the button during its {@link edu.wpi.first.wpilibj.command.Scheduler#run()}.
     *
    ...
    /**
     * Adds a command immediately to the {@link edu.wpi.first.wpilibj.command.Scheduler}. This should only be
     * called in the {@link edu.wpi.first.wpilibj.command.Scheduler#run()} loop. Any command with conflicting
     * requirements will be removed, unless it is uninterruptable. Giving
     * <code>null</code> does nothing.
     *
     * @param command the {@link edu.wpi.first.wpilibj.command.Command} to add
     */
        ...
        if (!commandTable.containsKey(command)) {
            ...
            while (requirements.hasMoreElements()) {
                ...
                if (lock.getCurrentCommand() != null) {
                    System.out.println(lock.getCurrentCommand().getName() + " command cancelled!");
     ...
     * in order to have a functioning {@link edu.wpi.first.wpilibj.command.Command} system. The loop has five
     * stages:
     * <p/>
    ...
    /**
     * Registers a {@link Subsystem} to this {@link edu.wpi.first.wpilibj.command.Scheduler}, so that the
     * {@link edu.wpi.first.wpilibj.command.Scheduler} might know if a default {@link edu.wpi.first.wpilibj.command.Command} needs to be
    ...
    /**
     * Removes the {@link edu.wpi.first.wpilibj.command.Command} from the {@link edu.wpi.first.wpilibj.command.Scheduler}.
     *
