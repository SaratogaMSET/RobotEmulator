...
public class Relay extends SensorBase implements IDeviceController, LiveWindowSendable {
    ...
    private boolean initInGui;

    public Relay(int moduleNumber, int channel, Direction direction, boolean initInGui) {
        if (direction == null)
            throw new NullPointerException("Null Direction was given");
        m_channel = channel;
        m_direction = direction;
        this.initInGui = initInGui;
    ...
    public static class Value {
        ...
        public static final int kOn_val = 1;
        public static final int kForward_val = 2;
        public static final int kReverse_val = 3;
        /**
         * value: off
         */
    ...
    private void initRelay(final int moduleNumber) {
        ...
        if (initInGui)
            RobotEmulator.getInstance().addRelay(this);
    }
    ...
    public void set(Value value) {
        if (initInGui)
            RobotEmulator.getInstance().setRelayPosition(this, value);
        switch (value.value) {
            case Value.kOff_val:
                if (m_direction == Direction.kBoth || m_direction == Direction.kForward) {
                    m_module.setRelayForward(m_channel, false);
